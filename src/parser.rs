...
fn match_equation_internal(
    ...
    // === PREFILTER STEP ===
    // If constraints are present, build a constraint-aware prefilter on the fly;
    // otherwise use the cached prefilter from ParsedForm.
    let pass = if constraints.is_some() {
        let pattern_str = format!(
            "^{}$",
            form_to_regex_str_with_constraints(&parsed_form.parts, constraints)
        );
        let prefilter = get_regex(&pattern_str).unwrap_or_else(|_| parsed_form.prefilter.clone());
        prefilter.is_match(word).unwrap_or(false)
    } else {
        parsed_form.prefilter.is_match(word).unwrap_or(false)
    };
...
fn form_to_regex_str_with_constraints(
    parts: &[FormPart],
    constraints: Option<&VarConstraints>,
) -> String {
    use std::fmt::Write;
